<template>
    <div class="slider-container">
      <div class="slider-labels">
        <div v-for="i in 10" :key="i" class="label">
          <span>{{ 11 - i }}</span> <!-- Anzeige der Werte von 10 bis 1 -->
        </div>
      </div>
      <q-slider
        v-model="model"
      ...
      <div class="slider-labels-right">
        <div v-for="i in 2" :key="i" class="label">
          <span v-if="i === 1">{{ textAtTop }}</span>
          <span v-if="i === 2">{{ textAtBottom }}</span>
        </div>
      </div>
    </div>
  </template>
  ...
  export default {
    ...
    setup(props, { emit }) {
      const model = ref(props.modelValue || 10); // Standardwert auf 10 setzen
  
      // Watcher, um das Modell zu beobachten und Events zu emittieren
